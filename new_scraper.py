    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            with open(csv_file_path, mode='r', encoding='utf-8') as file:
                ...
                meet_date = rows[1][0]
                meet_url = rows[2][0]
                meet_comments = rows[3][0]
                
                # Start reading team results from row 6 onwards
                team_results = rows[6:]  # Team place, name, and score
...
<head>
  ...
  <link rel="stylesheet" href="css/style.css">
    ...
    """
                ...
                for row in team_results:
                    if len(row) == 8:  # Ensure valid row with 3 columns
                        place, grade, name, athlink, time, team, team_link, pro_pic = row
                        page += f"""<tr>
                        <td>{place}</td>
                        <td>{grade}</td>
